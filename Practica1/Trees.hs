...
nNodes :: BTree a -> Int
nNodes Void = 0
nNodes (Node nodo left right) = 1 + nNodes left + nNodes right

-- | Regresa el número de hojas de un árbol.
nLeaves :: BTree a -> Int
nLeaves (Node nodo Void Void) = 1
nLeaves (Node nodo left right) = nLeaves left + nLeaves right

-- | Regresa el número de nodos internos de un árbol.
nni :: BTree a -> Int
nni (Node nodo Void Void) = 0
nni (Node nodo left right) = 1 + nni left + nni right

-- | Nos dice si un elemento está contenido en un árbol ordenado.
contains :: (Ord a, Eq a) => a -> BTree a -> Bool
contains a Void = False
contains a (Node nodo left right) = if nodo == a then True else (contains a left || contains a right)

-- | Recorrido inorder.
inorder :: BTree a -> [a]
inorder Void = []
inorder (Node nodo left right) = inorder left ++ [nodo] ++ inorder right

-- | Recorrido preorder.
preorder :: BTree a -> [a]
preorder Void = []
preorder (Node nodo left right) = [nodo] ++ preorder left ++ preorder right

-- | Recorrido postorder.
postorder :: BTree a -> [a]
postorder Void = []
postorder (Node nodo left right) = postorder left ++ postorder right ++ [nodo]

-- | Agrega un elemento a un árbol binario de manera ordenada.
add :: (Ord a) => a -> BTree a -> BTree a
add a Void = (Node a Void Void)
add a (Node nodo left right)
 | a <= nodo = if left == Void then (Node nodo (Node a Void Void) right) else (Node nodo (add a left) right)
 | a >= nodo = if right == Void then (Node nodo left (Node a Void Void)) else (Node nodo left (add a right))

-- | Pasa una lista a un árbol binario de forma ordenada.
fromList :: (Ord a) => [a] -> BTree a -> BTree a
fromList [] (Node nodo left right) = (Node nodo left right)
fromList listita arbolito = fromList (tail listita) (add (head listita) arbolito)
